...
def run(method, seed):
    ...
    acc_after=[]; P=None; Pref=None; mu_log=[]; gmem=[]
    for t in range(args.ntasks):
        ...
        # Read-out reference basis: fixed from the FIRST task representation, retained.
        # Coherence mu is measured for EVERY method (not only projection ones),
        # as the alignment of the current representation with this maintained subspace.
        if Pref is None:
            Pref=feat_basis(net,perm)
        with torch.no_grad():
            hs=[]; n=0
            for x,_ in batches(TRX,TRY,1000,False):
            ...
            mu_log.append((Pref@H.t()).norm(dim=0).mean().item())
        if method=='OGD':
            net.eval()
