...
def train_task_derpp(net,perm,epochs,buf,alpha,beta,cap=1000):
    opt=torch.optim.SGD(net.parameters(),lr=0.1,momentum=0.9); ce=nn.CrossEntropyLoss(); mse=nn.MSELoss(); net.train()
    seen=buf['n']
    for ep in range(epochs):
        for x,y in batches(Xtr[:,perm],Ytr):
            opt.zero_grad(); out=net(x); loss=ce(out,y)
            if len(buf['x'])>0:
                import random as _r; idxs=torch.randint(0,len(buf['x']),(min(128,len(buf['x'])),))
                bx=torch.stack([buf['x'][k] for k in idxs]).to(dev)
                bl=torch.stack([buf['l'][k] for k in idxs]).to(dev)
                by=torch.stack([buf['y'][k] for k in idxs]).to(dev)
                bo=net(bx); loss=loss+alpha*mse(bo,bl)+beta*ce(bo,by)
            loss.backward(); opt.step()
            for i in range(x.shape[0]):
                buf['n']+=1; import random as _r2
                if len(buf['x'])<cap:
                    buf['x'].append(x[i].detach().cpu()); buf['l'].append(out[i].detach().cpu()); buf['y'].append(y[i].detach().cpu())
                else:
                    j=_r2.randint(0,buf['n']-1)
                    if j<cap: buf['x'][j]=x[i].detach().cpu(); buf['l'][j]=out[i].detach().cpu(); buf['y'][j]=y[i].detach().cpu()

def train_task_trgp(net,perm,epochs,Bfull,Btrust,scale):
    opt=torch.optim.SGD(net.parameters(),lr=0.1,momentum=0.9); ce=nn.CrossEntropyLoss(); net.train()
    Pf=None if Bfull is None else Bfull.T@Bfull
    Pt=None if Btrust is None else Btrust.T@Btrust
    for ep in range(epochs):
        for x,y in batches(Xtr[:,perm],Ytr):
            opt.zero_grad(); ce(net(x),y).backward()
            ...
            if Pf is not None:
                gp=g-Pf@g
                if Pt is not None: gp=gp+scale*(Pt@g)
                net.f2.weight.grad[:]=gp
            if net.head.weight.grad is not None and Bfull is not None:
                net.head.weight.grad[:]=proj_out(net.head.weight.grad,Bfull)
            opt.step()
...
def run(method,seed,perms,tau):
    ...
    for t in range(args.ntask):
        ...
        if method=="TRGP":
            ...
            prelim=feat_basis(net,perm,args.energy,args.cap); tr=[]
            for s in range(t):
                if tb[s] is None: continue
                if subspace_overlap(prelim,tb[s])>=tau: tr.append(tb[s])
            Btrust=ortho(tr); Bprot=Bfull
        if method=="DERPP":
            train_task_derpp(net,perm,args.epochs,buf,0.5,0.5)
        elif method=="TRGP":
            train_task_trgp(net,perm,args.epochs,Bprot,Btrust,0.5)
        else:
...
if __name__=='__main__':
    ...
    for seed in range(args.seeds):
        for m in ['FINETUNE','GPM','TRGP','DERPP','COBOUND']:
