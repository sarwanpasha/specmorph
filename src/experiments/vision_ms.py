...
def mstd(v):
    ...
    return m,s
print('MULTISEED', 'seeds=%d'%args.seeds, 'lam=%.2f'%args.lam, 'epochs=%d'%args.epochs, 'dev=%s'%dev)
METHODS=['FINETUNE','OGD','GPM','OURS']
agg={m:{'avg':[],'forget':[],'mu':[],'aa':[]} for m in METHODS}
for s in range(args.seeds):
    for m in METHODS:
        t0=time.time(); avg,forget,mu,aa=run(m,s)
        agg[m]['avg'].append(avg); agg[m]['forget'].append(forget); agg[m]['aa'].append(aa)
        if not math.isnan(mu): agg[m]['mu'].append(mu)
        print(f'seed{s} {m:9} avg={avg:.4f} forget={forget:.4f} mu={mu:.4f} accAfter={aa:.4f} [{time.time()-t0:.0f}s]', flush=True)
print('='*72)
print(f'{"method":9}{"avg_acc":>16}{"forget":>16}{"mu":>12}{"accAfter":>12}')
for m in METHODS:
    am,asd=mstd(agg[m]['avg']); fm,fsd=mstd(agg[m]['forget']); aam,aasd=mstd(agg[m]['aa'])
    if agg[m]['mu']:
        mm,msd=mstd(agg[m]['mu']); mustr=f'{mm:.3f}+-{msd:.3f}'
    else:
        mustr='---'
    print(f'{m:9}{am:.4f}+-{asd:.4f}   {fm:.4f}+-{fsd:.4f}   {mustr:>12}  {aam:.3f}')
print('='*72)
print('Done.')
