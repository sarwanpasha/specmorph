...
if __name__=='__main__':
    ...
    for t in range(NTASK):
        ...
        opt=torch.optim.Adam([W,b],lr=0.01,weight_decay=1e-4)
        for ep in range(15):
            for i in range(0,len(Xi),512):
                opt.zero_grad(); F.cross_entropy(Xi[i:i+512]@W.t()+b, Yi[i:i+512]).backward(); opt.step()
    ...
    rel_drop=drop0/max(acc0_init,1e-6)
    RHS=sin_agg*mu_max
    print(f'SEED {seed} frozen ViT-B/16 shared-head NTASK={NTASK}',flush=True)
